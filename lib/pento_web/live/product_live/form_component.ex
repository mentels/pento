defmodule PentoWeb.ProductLive.FormComponent do
  ...
  defp save_product(socket, :edit, product_params) do
    socket
    |> params_with_image(product_params)
    |> then(&Catalog.update_product(socket.assigns.product, &1))
    |> case do
      {:ok, product} ->
  ...
  defp params_with_image(socket, params) do
    ...
    Map.put(params, "image", path)
  end

  defp upload_static_file(%{path: path}, _entry) do
    File.cp!(
      path,
      Path.join([Application.app_dir(:pento), "priv/static/images", Path.basename(path)])
    )
